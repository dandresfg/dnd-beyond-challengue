...
export const Sidebar = ({ logs }: SidebarProps) => {
    ...
    const getLogColor = (type: LogEntry['type']) => {
        switch (type) {
            case 'attack': return styles.attackLog;
            case 'heal': return styles.healLog;
            case 'tempHp': return styles.tempLog;
        }
    };
                    ...
                    <Flex direction="column" gap={2}>
                        {logs.map((log) => (
                            <div key={log.id} className={`${styles.logEntry} ${getLogColor(log.type)}`}>
                                ...
                                {log.enemy ? (
                                    <Flex direction="row" align="center" gap={1} className={styles.enemyInfo}>
                                        <Text variant="label" className={styles.enemyName}>{log.enemy.name}</Text>
                                        <Text variant="body" className={styles.logMessage}>{log.message}</Text>
                                    </Flex>
                                ) : (
                                    <Text variant="body" className={styles.logMessage}>{log.message}</Text>
                                )}
